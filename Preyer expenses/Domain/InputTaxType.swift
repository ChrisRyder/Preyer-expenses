...
class InputTaxType: Object , Uploadable{
    ...
    static var resourceURL: URL {
        return URL(string: "\(BASE_APP_URL)/api/inputTaxTypes")!
    }
    ...
    static func getList() {
        ...
        sessionManager.request(resourceURL).responseJSON { (response: DataResponse<Any>) in
            ...
            if response.error == nil {
                ...
                do {
                    let objList = try JSONDecoder().decode([InputTaxType].self, from: response.data!)
                    //print ("InputTaxTypeList: \(String(describing:objList))")
